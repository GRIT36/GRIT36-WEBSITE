...
function historySummaryShortcut(){
 if(!state.session?.finished||!["study","math","geography","reading"].includes(state.view))return;
 const summary=document.querySelector("#app .summary");if(!summary||document.getElementById("openHistory"))return;
 const container=document.createElement("div");container.className="history-entry";
 const button=document.createElement("button");button.className="btn";button.id="openHistory";button.textContent="📊 Leerhistorie";container.appendChild(button);summary.appendChild(container)
}
function historyHandlers(){
 ...
 document.getElementById("openHistory")?.addEventListener("click",()=>{clearCelebration();stopSound();stopStudyMusic();readingStop();readingRecordDispose();state.view="history";render();loadLearningHistory()});
