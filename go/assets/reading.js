...
function readingSpeak(){
 if(!readingRound||state.session?.finished||document.hidden)return;
...
function readingMenu(){return `<button class="back" id="readingHome">‹ Terug naar Max</button><h1>📖 Hardop lezen</h1><p class="hint">Luister, lees zelf en kies een kleur. Tik op de zin voor de Chinese betekenis.</p>${READING_LESSONS.map(l=>`<section class="accuracy-panel"><h2>Les ${l.number} · ${esc(l.title)}</h2><div class="reading-parts">${["A","B"].map(part=>`<button class="btn primary" data-reading-lesson="${l.number}" data-reading-part="${part}">${part} · ${part==="A"?l.split:l.sentences.length-l.split} zinnen</button>`).join("")}</div></section>`).join("")}`}
function requestReading(lesson,part){readingPending={lesson:Number(lesson),part};state.pendingMode="reading";state.pendingLearner=state.session?.learner||"Max";state.modal="learner";render()}
function readingStart(learner){
 const lesson=READING_LESSONS.find(l=>l.number===readingPending.lesson),part=readingPending.part;
 readingStop();stopSound();stopStudyMusic();homeMusicPause();clearCelebration();historyIndex=null;
 const sentences=part==="A"?lesson.sentences.slice(0,lesson.split):lesson.sentences.slice(lesson.split);
 readingRound={lesson:lesson.number,part,title:`Hardop lezen · Les ${lesson.number}${part} · ${lesson.title}`,sentences,index:0,shown:false,ratings:{}};
 ...
 beginLearningRecord(learner,{id:`reading:Max:${lesson.number}:${part}`,name:readingRound.title},"flashcard");render();readingSpeak()
}
function readingRate(level){
 if(!readingRound||state.session.finished||!READING_RATINGS[level])return;
...
function readingMove(step){
 const r=readingRound;if(!r||state.session.finished)return;
 if(step>0&&!r.ratings[r.sentences[r.index].id])return;
 if(r.index+step<0)return;
 if(r.index+step>=r.sentences.length){
  if(state.session.wordResults.length!==r.sentences.length)return;
  readingStop();stopSound();updateLearningClock();state.session.finished=true;state.session.recordDuration=learningSnapshot().duration_seconds;finalizeLearningRecord(true);render();return
 }
 readingStop();stopSound();r.index+=step;r.shown=false;checkpointLearning();render();readingSpeak()
}
...
function readingPage(){
 ...
 if(s.finished)return `<div class="summary-radiance" aria-hidden="true"></div><div class="reading-study summary"><div class="summary-finale"><div class="summary-fireworks" aria-hidden="true"><i class="firework finale-left"></i><i class="firework finale-right"></i></div><h2>🎉 Goed geoefend!</h2></div><h2>${esc(r.title)}</h2><p>${s.total} zinnen geoefend · jouw eigen beoordeling</p>${recordSaveStatus()}${readingGroups(s.wordResults)}<div class="reading-parts"><button class="btn" id="readingAgain">Nog een keer</button>${r.part==="A"?'<button class="btn primary" id="readingB">Verder met B →</button>':""}<button class="btn" id="readingMenu">Alle lessen</button></div></div>`;
 const q=r.sentences[r.index],level=r.ratings[q.id];
 return `<div class="reading-study"><div class="learner-header"><div><div class="learner-name">${learnerLabel(s.learner)}</div><button class="back" id="readingExit">‹ Afsluiten</button></div><button class="btn" id="readingSound" aria-pressed="${audioSettings.sound}">🔊 Geluid ${audioSettings.sound?"aan":"uit"}</button></div><h2>Les ${r.lesson} · ${r.part} · ${esc(q.story)}</h2><p class="meta">${r.index+1} / ${s.total} · ${s.wordResults.length} beoordeeld</p><progress class="math-progress" max="${s.total}" value="${s.wordResults.length}" aria-label="Voortgang"></progress><button class="reading-card" id="readingText" aria-expanded="${r.shown}"><span lang="nl">${esc(q.nl)}</span>${r.shown?`<span class="reading-chinese" lang="zh">${esc(q.zh)}</span>`:""}</button><p class="hint">Tik op de zin voor de Chinese betekenis.</p><button class="btn primary" id="readingListen">🔊 Luisteren</button><p id="readingAudioStatus" role="status"></p><p>Hoe ging het lezen?</p><div class="fc-buttons">${Object.entries(READING_RATINGS).map(([key,info])=>`<button class="btn memory-${key}" data-reading-rating="${key}" aria-pressed="${key===level}">${info.icon} ${info.label}</button>`).join("")}</div><p class="hint">${level?"Klik op Volgende als je klaar bent.":"Kies een kleur om verder te gaan."}</p><div class="reading-navigation"><button class="btn" id="readingPrevious" ${r.index===0?"disabled":""}>⬅️ Vorige</button><button class="btn primary" id="readingNext" ${level?"":"disabled"}>${r.index===s.total-1?"Afronden ✓":"Volgende ➡️"}</button></div></div>`
}
function readingLeave(){readingStop();stopSound();finalizeLearningRecord();readingRound=null;state.session=null;state.mode=null;state.view="readingMenu";render()}
function readingHandlers(){
 document.getElementById("openReading")?.addEventListener("click",()=>{homeMusicPause();state.view="readingMenu";render()});
 document.getElementById("readingHome")?.addEventListener("click",()=>{state.view="home";state.filter="Jongste";render()});
 ...
 document.getElementById("readingB")?.addEventListener("click",()=>requestReading(readingRound.lesson,"B"));
