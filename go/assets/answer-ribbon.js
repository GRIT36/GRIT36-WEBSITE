...
function ribbonQuestion(){return state.view==="math"?mathRound?.index:state.session?.queue?.[0]?.id}
function clearAnswerRibbon(){clearTimeout(answerRibbonTimer);answerRibbon?.remove();answerRibbon=null;answerRibbonOwner=null}
function checkAnswerRibbon(){if(answerRibbon&&(answerRibbonOwner!==state.session||state.session?.finished||!["study","math"].includes(state.view)||answerRibbonQuestion!==ribbonQuestion()||(state.view==="study"&&historyIndex!==null)))clearAnswerRibbon()}
function showAnswerRibbon(correct){
 ...
 const header=document.querySelector(state.view==="math"?".math-progress":".session-top");
